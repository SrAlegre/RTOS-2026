...
void sem_wait(sem_t *sem) {
    ...
    if (sem->contador < 0) {
        ...
        sem->pos_input++;
    if (sem->pos_input >= MAX_USER_TASKS)
        sem->pos_input = 0;
...
void sem_post(sem_t *sem) {
    ...
    if (sem->contador <= 0) {
        ...
        SET_STATE(r_queue.TASKS[sem->fila[sem->pos_output]], READY);
...
void mutex_unlock(mutex_t *m) {
    ...
    if(m->locked && m->owner_id == r_queue.TASKS[r_queue.pos_task_running].task_id)
    {
        if(m->waiting_count>0){
            ...
            SET_STATE(r_queue.TASKS[task_idx], READY);
